        ...
        public void HouseFurnitures_ClientEvent(Client player, string event_name, params object[] args)
        {
            switch (event_name)
            {
                ...
                case "HouseBuyFurniture":
                {
                    ...
                    if (API.exported.MoneyAPI.GetMoney(player) < FurnitureList[idx].Price)
                    {
                        player.sendNotification("Error", "~r~You can't afford this furniture.");
                        return;
                    }

                    API.exported.MoneyAPI.ChangeMoney(player, -FurnitureList[idx].Price);
